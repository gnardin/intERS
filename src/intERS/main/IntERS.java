	...
	public static void main(String[] args) throws Exception {
		...
		for(int sim = 0; sim < simulation.getNumberRuns(); sim++) {
			...
			runner.load((sim + 1), randomSeed, simulation.getRSDirectory(),
					simulation.getXMLFilename(), simulation.getXSDFilename(), simulation
							.getOutput().getDirectory(), simulation.getOutput()
							.getFileExtorter(), simulation.getOutput().getFileObserver(),
					simulation.getOutput().getFileTarget(), simulation.getOutput()
							.getFileAppend(), simulation.getOutput().getFieldSeparator(),
		...
		DataAggregation summary = new DataAggregation(simulation.getNumberRuns(),
				simulation.getOutput().getDirectory(), simulation.getOutput()
						.getFieldSeparator());
		
		summary.aggregate(simulation.getOutput().getClassExtorterStat(), simulation
				.getOutput().getFileExtorter(), simulation.getOutput()
				.getFilePrefixAvg() + simulation.getOutput().getFileExtorter(),
				simulation.getOutput().getFilePrefixSum()
						+ simulation.getOutput().getFileExtorter());
		
		summary.aggregate(simulation.getOutput().getClassObserverStat(), simulation
				.getOutput().getFileObserver(), simulation.getOutput()
				.getFilePrefixAvg() + simulation.getOutput().getFileObserver(),
				simulation.getOutput().getFilePrefixSum()
						+ simulation.getOutput().getFileObserver());
		
		summary.aggregate(simulation.getOutput().getClassTargetStat(), simulation
				.getOutput().getFileTarget(), simulation.getOutput().getFilePrefixAvg()
				+ simulation.getOutput().getFileTarget(), simulation.getOutput()
				.getFilePrefixSum() + simulation.getOutput().getFileTarget());
	}
}