...
nLL <- eLL[,numPunishment/numExtortion]
nLL[is.nan(nLL)] <- 0

nLH <- eLH[,numPunishment/numExtortion]
nLH[is.nan(nLH)] <- 0

nHL <- eHL[,numPunishment/numExtortion]
nHL[is.nan(nHL)] <- 0

nHH <- eHH[,numPunishment/numExtortion]
nHH[is.nan(nHH)] <- 0
...
png(file=paste(imagePath,"/numberFightXExtortion.png", sep=""),
    width=800, height=600)
grid.arrange(plotLL, plotLH, plotHL, plotHH, main="Number of Fight X Extortion")
dev.off()
...
png(file=paste(imagePath,"/numberFightXExtortion",ext,".png", sep=""),
    width=800, height=600)
grid.arrange(plotLL, plotLH, plotHL, plotHH, main="Number of Fight X Extortion")
dev.off()