...
nLL <- eLL[,(numReceivedRetaliation-numCounterattack)/numReceivedRetaliation]
nLL[is.nan(nLL) | is.infinite(nLL)] <- 0

nLH <- eLH[,(numReceivedRetaliation-numCounterattack)/numReceivedRetaliation]
nLH[is.nan(nLH) | is.infinite(nLH)] <- 0

nHL <- eHL[,(numReceivedRetaliation-numCounterattack)/numReceivedRetaliation]
nHL[is.nan(nHL) | is.infinite(nHL)] <- 0

nHH <- eHH[,(numReceivedRetaliation-numCounterattack)/numReceivedRetaliation]
nHH[is.nan(nHH) | is.infinite(nHH)] <- 0
...
data <- data.table(extorter="LL",type="Attacked",value=mean(nLL[1:100]))
data <- rbind(data,data.table(extorter="LH",type="Attacked",value=mean(nLH[1:100])))
data <- rbind(data,data.table(extorter="HL",type="Attacked",value=mean(nHL[1:100])))
data <- rbind(data,data.table(extorter="HH",type="Attacked",value=mean(nHH[1:100])))
