...
nLL <- eLL[1:iLL-1][numHelpRequested > 0,]
nLH <- eLH[1:iLH-1][numHelpRequested > 0,]
nHL <- eHL[1:iHL-1][numHelpRequested > 0,]
nHH <- eHH[1:iHH-1][numHelpRequested > 0,]

data <- data.table(type="LL",
                   value=mean((nLL$numHelpRequest-nLL$numReceivedCounterattack)/nLL$numHelpRequest))
data <- rbind(data, data.table(type="LH",
                               value=mean((nLH$numHelpRequest-nLH$numReceivedCounterattack)/nLH$numHelpRequest)))
data <- rbind(data, data.table(type="HL",
                               value=mean((nHL$numHelpRequest-nHL$numReceivedCounterattack)/nHL$numHelpRequest)))
data <- rbind(data, data.table(type="HH",
                               value=mean((nHH$numHelpRequest-nHH$numReceivedCounterattack)/nHH$numHelpRequest)))


ggplot(data, aes(factor(type, levels=c("LL","LH","HL","HH")),  y=value*100)) +
  ylim(0,100) +
  labs(title = "") + xlab("Extorter Policy") + ylab("% Protection") +
  geom_bar(width=0.5) +
